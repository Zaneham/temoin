...
let mp = Runner.Test {
  ...
  parties = 2;
  labels = [| "sawflag"; "data" |];
  radix = 2; init = fresh; reset = clear; control = false;
  thread = (fun s i ->
    if i = 0 then begin
      s.data.(slot) <- 1;
...
let wrc = Runner.Test {
  ...
  observe = (fun s o ->
    o.(0) <- s.res.(0); o.(1) <- s.res.(1); o.(2) <- s.res.(2));
...
let iriw = Runner.Test {
  ...
  observe = (fun s o ->
    o.(0) <- s.res.(0); o.(1) <- s.res.(1);
    o.(2) <- s.res.(2); o.(3) <- s.res.(3));
  forbidden = (fun o ->
...
let cas = Runner.Test {
  ...
  radix = 3; init = fresh; reset = clear; control = false;
  thread = (fun s i ->
    let won = Atomic.compare_and_set s.x 0 (i + 1) in
    s.res.(i) <- (if won then 1 else 0));
  observe = (fun s o ->
    o.(0) <- s.res.(0); o.(1) <- s.res.(1); o.(2) <- Atomic.get s.x);
  forbidden = (fun o ->
    (o.(0) = 1 && o.(1) = 1)
    || (o.(0) = 0 && o.(1) = 0)
    || (o.(0) = 1 && o.(2) <> 1)
